...
test.describe('Cart and checkout regression', () => {
  ...
  test('deleting a cart item removes it from the visible list', { tag: ['@regression', '@cart'] }, async ({
    page,
    productDetailPage,
    cartPage,
  }) => {
  ...
  test('signing up with an already-registered username is rejected', { tag: ['@regression', '@auth'] }, async ({
    homePage,
    env,
  }) => {
    await homePage.open();
